...
from .pipeline_components.crf_label_as_entity_extractor import CRFLabelAsEntityExtractor  # NOQA: E501
...
registry.registered_components = {
    c.name: c for c in registry.component_classes
}
...
def register_component_alias(component_name, component_class):
    class OutputClass(component_class):
        name = component_name
    registry.registered_components.update({
        component_name: OutputClass,
    })
...
register_component_alias(
    'bothub_nlp.core.pipeline_components.spacy_nlp.SpacyNLP',
    SpacyNLP,
)
register_component_alias(
    'bothub_nlp.core.pipeline_components.tokenizer_spacy.SpacyTokenizer',
    SpacyTokenizer,
)
