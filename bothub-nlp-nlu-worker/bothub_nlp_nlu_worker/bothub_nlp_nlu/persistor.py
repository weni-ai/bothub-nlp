...
class BothubPersistor(Persistor):
    def __init__(self, update=None, repository_authorization=None, *args,
                 **kwargs):
    ...
    def retrieve(self, model_name, project, target_path):
        ...
        tar_data = base64.b64decode(
            self.backend().request_backend_parse_nlu_persistor(
                self.update,
                self.repository_authorization).get('bot_data')
            )
