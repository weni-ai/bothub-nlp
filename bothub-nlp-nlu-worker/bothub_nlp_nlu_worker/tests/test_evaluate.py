import unittest

from bothub_nlp_celery.actions import queue_name, ACTION_EVALUATE
from bothub_nlp_celery.app import celery_app
from bothub_nlp_celery.tasks import TASK_NLU_EVALUATE_UPDATE

from bothub_nlp_nlu_worker.bothub_nlp_nlu.utils import backend


class TestEvaluateTask(unittest.TestCase):
    def setUp(self):
        language = 'pt_br'

        repository_authorization = ''  # TODO: UUID Integration

        current_update = backend().request_backend_parse(
            router="evaluate", repository_authorization=repository_authorization, language=language
        )

        self.answer_task = celery_app.send_task(
            TASK_NLU_EVALUATE_UPDATE,
            args=[
                current_update.get("repository_version"),
                current_update.get("user_id"),
                repository_authorization,
            ],
            queue=queue_name(ACTION_EVALUATE, current_update.get("language")),
        )
        self.answer_task.wait()
        self.response = self.answer_task.result

    def test_evaluate_ok(self):
        print(f"Response Evaluate: {self.response}")
        self.assertEqual(len(self.response), 2)


